...
const CartPage = () => {
  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}carts/${item._id}`, {
        method: "PUT",
        headers: {
    ...
    if (item.quantity > 1) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}carts/${item._id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDelete =   (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${import.meta.env.VITE_BACKEND_URL}carts/${item._id}`).then(response => {
          if (response) {
            refetch();
